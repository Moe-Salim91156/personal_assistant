...
PLUGIN_DIR = SCRIPT_DIR.parent.parent  # plugins/run_scripts/
PROJECT_ROOT = PLUGIN_DIR.parent.parent  # personal_assistant/
REF_BASE = PROJECT_ROOT / "references"
...
def highlight_code_line(line):
    ...
    stripped = re.sub(
        keywords, lambda m: colorize(m.group(), Colors.BOLD, Colors.MAGENTA), stripped
    )

    stripped = re.sub(
        r'"(?:[^"\\]|\\.)*"', lambda m: colorize(m.group(), Colors.GREEN), stripped
    )
    stripped = re.sub(
        r"'(?:[^'\\]|\\.)*'", lambda m: colorize(m.group(), Colors.GREEN), stripped
    )
    ...
    stripped = re.sub(
        r"(\w+)(\s*\()",
        lambda m: colorize(m.group(1), Colors.YELLOW) + m.group(2),
        stripped,
    )
...
def search_references(query):
    ...
    if found:
        for topic, category, matches in found:
            print(
                colorize(f"  📄 {topic}", Colors.BOLD, Colors.GREEN)
